def is_move_valid(filed, next_row, next_col, rows, cols):
...
for command in commands:
    ...
    if is_move_valid(playing_field, player_next_row, player_next_col, rows, cols):
        if playing_field[player_next_row][player_next_col] == BUNNY:
            ...
            for position in rabbits_positions:
                ...
                for move in moves:
                    ...
                    if is_move_valid(playing_field, rabbit_next_row, rabbit_next_col, rows, cols):
            ...
            playing_field[player_current_row][player_current_col] = FREE_SPACE
    ...
    for position in rabbits_positions:
        ...
        for move in moves:
            ...
            if is_move_valid(playing_field, rabbit_next_row, rabbit_next_col, rows, cols):
                if playing_field[rabbit_next_row][rabbit_next_col] == PLAYER:
                    player_final_row, player_final_col = rabbit_next_row, rabbit_next_col
                    playing_field[rabbit_next_row][rabbit_next_col] = BUNNY
                    is_dead = True
                else:
                    playing_field[rabbit_next_row][rabbit_next_col] = BUNNY
...
# First move the player, but first check if player next move is on bunny
# Then spread the bunnies
